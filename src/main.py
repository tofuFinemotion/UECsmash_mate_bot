...
# ファイル名をパスに結合
data_file = os.path.join(data_path, "user_data.json")
matching_room_file = os.path.join(data_path, "matching_room.json")
matching_standby_file = os.path.join(data_path, "matching_standby.json")
...
def delete_matching_room(room_data, user_id):
    ...
    if match_id is not None:
        ...
        with open(matching_room_file, "w") as file:
...
# マッチング情報を取得する関数
def room_information(room_id, user_id, user_data, room_data):
    opponent_id = get_opponent(user_id, room_data)
    
    # opponent_idがNoneの場合のチェック
    if opponent_id is None:
        return "対戦相手が見つかりませんでした。"

    # user_dataにユーザー情報が存在するか確認
    if user_id not in user_data or opponent_id not in user_data:
        return "ユーザー情報が見つかりませんでした。"

    return (
        f"部屋ID: {room_id if room_id else f'部屋IDが登録されていません。<@{user_id}>が部屋を建ててください。'}\n"
        f"レート情報:\n"
        f"<@{user_id}>: rating: {user_data[user_id]['rating']}, 拒否ステージ: {', '.join(user_data[user_id]['ban_stages'])}\n"
        f"<@{opponent_id}>: rating: {user_data[opponent_id]['rating']}, 拒否ステージ: {', '.join(user_data[opponent_id]['ban_stages'])}"
    )
...
async def win(ctx, winner: discord.Member):
    ...
    with open(matching_room_file, "r") as file:
...
async def match(ctx):
    ...
    # JSONファイルを読み込む
    with open(matching_standby_file, "r") as file:
    ...
    # `standby_data`がリストではなく辞書型かどうか確認
    if isinstance(standby_data, list):
        # 辞書に変換
        standby_data = {str(entry): {"room_id": None} for entry in standby_data}
        with open(matching_standby_file, "w") as file:
    ...
    if any(user_id in match["players"] for match in room_data.values()):
        ...
        room_id = match_info.get("room_id")  # match_infoからroom_idを取得
        await ctx.send(f"あなたは既にマッチング中です!")
        await ctx.send(room_information(room_id, user_id, user_data, room_data))
        return
    
    # 既に待機リストにいる場合、部屋IDをチェックして返す
    if user_id in standby_data:
        room_id = standby_data[user_id].get("room_id")
        if room_id:
            await ctx.send(f"あなたは既にマッチング待機リストに参加しています。登録されている部屋ID: {room_id}")
        else:
            await ctx.send("あなたは既にマッチング待機リストにいますが、部屋IDが登録されていません。`!room`コマンドで登録してください。")
        return

    # マッチング待機リストに追加
    if user_id not in standby_data:
        standby_data[user_id] = {"room_id": None}
        with open(matching_standby_file, "w") as file:
            json.dump(standby_data, file, ensure_ascii=False, indent=4)
    ...
    for opponent_id in standby_data:
        if opponent_id != user_id and user_data[opponent_id]["current_opponent"] != user_id:
            ...
            user_room_id = standby_data[user_id].get("room_id")
            opponent_room_id = standby_data[opponent_id].get("room_id")

            # 部屋IDの優先順位を設定（例: user_id側の部屋IDを優先）
            room_id = user_room_id if user_room_id else opponent_room_id

            room_data[match_id] = {
                "players": [user_id, opponent_id],
                "room_id": room_id  # 部屋IDを引き継ぐ
            }
            ...
            # 待機リストから削除
            del standby_data[user_id]
            del standby_data[opponent_id]
            with open(matching_standby_file, "w") as file:
                json.dump(standby_data, file, ensure_ascii=False, indent=4)
    ...
    await ctx.send("相手がいないか連戦になるため、マッチングできません。\n`!room`で部屋IDを登録してお待ちください。")
...
async def match_cancel(ctx):
    ...
    if user_id in standby_data:
        # 待機リストから削除
        del standby_data[user_id]  # 修正: remove()ではなくdelを使用
        with open(matching_standby_file, "w") as file:
            json.dump(standby_data, file, ensure_ascii=False, indent=4)
...
@bot.command(help="部屋IDを登録します。`!match`使用後に使用してください。")
async def room(ctx, room_id: str):
    user_id = str(ctx.author.id)
    ...
    if user_id in standby_data:
        ...
        with open(matching_standby_file, "w") as file:
            json.dump(standby_data, file, ensure_ascii=False, indent=4)
        await ctx.send(f"部屋ID「{room_id}」を待機リストに登録しました!")
        return
    ...
    if match_id:
        # マッチング中の部屋IDを登録または更新
        room_data[match_id]["room_id"] = room_id
        with open(matching_room_file, "w") as file:
            json.dump(room_data, file, ensure_ascii=False, indent=4)
        await ctx.send(f"部屋ID「{room_id}」をマッチングに登録しました!")
        await ctx.send(room_information(room_id, user_id, user_data, room_data))
        return

    await ctx.send("あなたは現在マッチング待機リストにもマッチング中にもいません。")
